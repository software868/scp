...
const HTM_GAS_ALARM = 'var(--color-critical)'
function htmGasAlarm(g: Gas): { legend: 'HIGH' | 'LOW'; phrase: string } | null {
  if (g.state === 'ok') return null
  if (g.vac) return { legend: 'LOW', phrase: 'INSUFFICIENT' }
  if (g.state === 'critical') return { legend: 'HIGH', phrase: 'PRESSURE HIGH' }
  return { legend: 'LOW', phrase: 'PRESSURE LOW' }
}
...
export default function App() {
  ...
  const [anae, setAnae] = useState({ run: false, sec: 0, target: 0 })
    ...
    <div className="flex h-screen max-h-dvh flex-col overflow-hidden bg-[var(--color-ink-950)] text-[var(--color-fg)]">
      ...
      <main className="min-h-0 flex-1 overflow-hidden">
        {page === 'home' ? (
          ...
          <Settings page={page} setPage={setPage} version={version} setVersion={setVersion} shared={shared} otOccupied={otOccupied} camActive={camActive} modelLabel={modelLabel} />
        )}
      </main>
...
function TimeCell({ label, value, tint, badge, onBadgeClick, controlsOpen, controls, className = '' }: { label: string; value: string; tint: string; badge?: string; onBadgeClick?: () => void; controlsOpen?: boolean; controls?: ReactNode; className?: string }) {
  const badgeActive = controlsOpen
  return (
    <div className={`relative flex h-full min-h-0 flex-col justify-between overflow-hidden rounded-2xl border px-2.5 py-2 sm:px-3 sm:py-2.5 ${className}`} style={{ borderColor: `color-mix(in srgb, ${tint} 34%, var(--color-line))`, background: `linear-gradient(155deg, color-mix(in srgb, ${tint} 10%, var(--color-ink-850)), var(--color-ink-900))` }}>
        ...
        {badge && (
          onBadgeClick ? (
            <button
              ...
              className="rounded-full border px-2 py-0.5 text-[9px] font-800 uppercase tracking-wide transition-all hover:brightness-125 active:scale-95 sm:text-[10px]"
              style={{ borderColor: `color-mix(in srgb, ${tint} 45%, transparent)`, color: tint, background: badgeActive ? `color-mix(in srgb, ${tint} 18%, transparent)` : 'transparent' }}
            >
              {badge}
            </button>
          ) : (
            <span className="rounded-full border px-2 py-0.5 text-[9px] font-800 uppercase tracking-wide sm:text-[10px]" style={{ borderColor: `color-mix(in srgb, ${tint} 45%, transparent)`, color: tint }}>{badge}</span>
          )
        )}
      </div>
      <div className="relative my-0.5 flex min-h-0 flex-1 flex-col items-center justify-center">
        <div className="w-full font-mono overview-timer-numeric font-800 leading-none tabular-nums" style={{ color: tint, textShadow: `0 0 18px color-mix(in srgb, ${tint} 35%, transparent)` }}>{value}</div>
      </div>
      {controls ? <div className="relative mt-0.5 flex shrink-0 gap-1 sm:gap-1.5">{controls}</div> : null}
    </div>
  )
}

function AnaesthesiaSetAlert({ open, tint, seconds, onAdjust, onDone, onClose }: { open: boolean; tint: string; seconds: number; onAdjust: (deltaSec: number) => void; onDone: () => void; onClose: () => void }) {
  if (!open) return null

  return (
    <>
      <button type="button" aria-label="Close duration set" className="fixed inset-0 z-40 bg-black/45" onClick={onClose} />
      <div
        className="absolute inset-x-0 top-0 z-50 rounded-2xl border p-4 shadow-[0_16px_48px_rgba(0,0,0,0.6)] sm:p-5"
        style={{
          borderColor: `color-mix(in srgb, ${tint} 40%, var(--color-line))`,
          background: `linear-gradient(155deg, color-mix(in srgb, ${tint} 12%, var(--color-ink-850)), var(--color-ink-950))`,
        }}
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          <span className="text-[12px] font-800 uppercase tracking-[0.14em] sm:text-[13px]" style={{ color: tint }}>Set Duration</span>
          <button
            type="button"
            aria-label="Close"
            onClick={onClose}
            className="grid h-7 w-7 place-items-center rounded-md border text-[var(--color-fg-soft)] transition-all hover:text-[var(--color-fg)] active:scale-95"
            style={{ borderColor: `color-mix(in srgb, ${tint} 35%, var(--color-line))` }}
          >
            <Icon.Close size={14} />
          </button>
        </div>
        <DurationSetPanel tint={tint} seconds={seconds} onAdjust={onAdjust} onDone={onDone} />
      </div>
    </>
  )
}

function DurationSetPanel({ tint, seconds, onAdjust, onDone }: { tint: string; seconds: number; onAdjust: (deltaSec: number) => void; onDone: () => void }) {
  const step = (label: string, onDown: () => void, onUp: () => void) => (
    <div className="flex min-w-0 flex-col items-stretch gap-2">
      <span className="text-center text-[11px] font-700 uppercase tracking-[0.12em] text-[#7f93a6]">{label}</span>
      <div className="grid grid-cols-2 gap-2">
        <button type="button" onClick={onDown} className="grid h-10 place-items-center rounded-lg border text-lg font-800 transition-all active:scale-90" style={{ borderColor: `color-mix(in srgb, ${tint} 45%, var(--color-line))`, color: tint }}>−</button>
        <button type="button" onClick={onUp} className="grid h-10 place-items-center rounded-lg border text-lg font-800 transition-all active:scale-90" style={{ borderColor: `color-mix(in srgb, ${tint} 45%, var(--color-line))`, color: tint }}>+</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <div className="text-center font-mono text-[2rem] font-800 tabular-nums leading-none sm:text-[2.25rem]" style={{ color: tint }}>{fmt(seconds)}</div>
      <div className="grid grid-cols-2 gap-4">
        {step('Hours', () => onAdjust(-3600), () => onAdjust(3600))}
        {step('Min', () => onAdjust(-60), () => onAdjust(60))}
      </div>
      <button
        type="button"
        onClick={onDone}
        className="min-h-[40px] rounded-lg border py-2 text-[12px] font-800 uppercase tracking-wide transition-all active:scale-95"
        style={{ borderColor: `color-mix(in srgb, ${tint} 45%, var(--color-line))`, color: tint, background: `color-mix(in srgb, ${tint} 14%, transparent)` }}
      >
        Done
      </button>
    </div>
...
function MiniBtn({ icon, label, onClick, tint }: { icon: ReactNode; label: string; onClick: () => void; tint: string }) {
  return (
    <button onClick={onClick} className="flex min-h-[28px] flex-1 items-center justify-center gap-1 rounded-md border px-2 py-0.5 text-[9px] font-800 uppercase tracking-wide transition-all active:scale-95 sm:min-h-[30px] sm:text-[10px]" style={{ borderColor: `color-mix(in srgb, ${tint} 40%, var(--color-line))`, color: tint, background: `color-mix(in srgb, ${tint} 10%, transparent)` }}>
      {icon} {label}
    </button>
  )
}
...
function GasTile({ g, accent, onClick }: { g: Gas; accent: string; onClick: () => void }) {
  const alarm = htmGasAlarm(g)
  const tint = alarm ? HTM_GAS_ALARM : accent
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={`${g.label} ${g.value} ${g.unit}${alarm ? `, ${alarm.legend}` : ', normal'}`}
      className={`group relative flex h-full min-h-0 w-full flex-col items-center justify-center gap-1 overflow-hidden rounded-xl border p-1.5 transition-all hover:-translate-y-0.5 active:scale-95 sm:p-2 ${alarm ? 'softpulse' : ''}`}
      style={{
        borderColor: `color-mix(in srgb, ${tint} ${alarm ? 70 : 32}%, var(--color-line))`,
        background: `linear-gradient(160deg, color-mix(in srgb, ${tint} ${alarm ? 22 : 12}%, var(--color-ink-850)), var(--color-ink-900))`,
      }}
    >
      <div className="pointer-events-none absolute -right-4 -top-4 h-16 w-16 rounded-full opacity-15 blur-2xl transition-opacity group-hover:opacity-30" style={{ background: `radial-gradient(circle, ${tint}, transparent 70%)` }} />
      <span className="relative max-w-full truncate px-0.5 text-center text-[clamp(0.75rem,1.8vh,0.95rem)] font-800 leading-tight" style={{ color: alarm ? tint : 'var(--color-fg)' }}>
        {g.label}
      </span>
      <span className="relative font-mono text-[clamp(1rem,2.6vh,1.45rem)] font-800 tabular-nums leading-none" style={{ color: tint }}>
        {g.value}
        <span className="ml-0.5 text-[clamp(0.55rem,1.2vh,0.7rem)] font-600">{g.unit}</span>
      </span>
      {alarm && (
        <span className="relative blink-critical text-[10px] font-800 uppercase tracking-widest" style={{ color: tint }}>
          {alarm.legend}
        </span>
      )}
    </button>
...
function ReadoutTile({ label, value, accent, onClick }: { label: string; value: string; accent: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative flex h-full min-h-0 w-full flex-col items-center justify-center gap-1 overflow-hidden rounded-xl border p-1.5 transition-all hover:-translate-y-0.5 active:scale-95 sm:p-2"
      style={{
        borderColor: `color-mix(in srgb, ${accent} 32%, var(--color-line))`,
        background: `linear-gradient(160deg, color-mix(in srgb, ${accent} 12%, var(--color-ink-850)), var(--color-ink-900))`,
      }}
    >
      <div className="pointer-events-none absolute -right-4 -top-4 h-16 w-16 rounded-full opacity-15 blur-2xl transition-opacity group-hover:opacity-30" style={{ background: `radial-gradient(circle, ${accent}, transparent 70%)` }} />
      <span className="relative max-w-full truncate px-0.5 text-center text-[clamp(0.95rem,2.6vh,1.35rem)] font-800 leading-tight text-[var(--color-fg)]">
        {label}
      </span>
      <span className="relative font-mono text-[clamp(1.05rem,3vh,1.6rem)] font-800 tabular-nums leading-none" style={{ color: accent }}>
        {value}
      </span>
    </button>
  )
...
function TheatreClockPanel({ now, gases, otOccupied, compact }: { now: Date; gases: Gas[]; otOccupied: boolean; compact?: boolean }) {
  ...
  return (
    <Panel pad={!compact} className={`${compact ? '!p-1.5 sm:!p-2' : ''} ${compact ? '' : 'home-deck'} relative flex h-full min-h-0 flex-col items-center justify-center overflow-hidden`}>
      <span className={`pointer-events-none absolute border-[var(--color-cyan)]/55 ${compact ? 'left-1.5 top-1.5 h-4 w-4 rounded-tl-md border-l-2 border-t-2 sm:left-2 sm:top-2' : 'left-2 top-2 h-5 w-5 rounded-tl-lg border-l-2 border-t-2 sm:left-3 sm:top-3 sm:h-6 sm:w-6'}`} style={{ borderColor: 'color-mix(in srgb, var(--color-cyan) 55%, transparent)' }} />
      <span className={`pointer-events-none absolute border-[var(--color-cyan)]/55 ${compact ? 'right-1.5 top-1.5 h-4 w-4 rounded-tr-md border-r-2 border-t-2 sm:right-2 sm:top-2' : 'right-2 top-2 h-5 w-5 rounded-tr-lg border-r-2 border-t-2 sm:right-3 sm:top-3 sm:h-6 sm:w-6'}`} style={{ borderColor: 'color-mix(in srgb, var(--color-cyan) 55%, transparent)' }} />
      <span className={`pointer-events-none absolute border-[var(--color-cyan)]/55 ${compact ? 'bottom-1.5 left-1.5 h-4 w-4 rounded-bl-md border-b-2 border-l-2 sm:bottom-2 sm:left-2' : 'bottom-2 left-2 h-5 w-5 rounded-bl-lg border-b-2 border-l-2 sm:bottom-3 sm:left-3 sm:h-6 sm:w-6'}`} style={{ borderColor: 'color-mix(in srgb, var(--color-cyan) 55%, transparent)' }} />
      <span className={`pointer-events-none absolute border-[var(--color-cyan)]/55 ${compact ? 'bottom-1.5 right-1.5 h-4 w-4 rounded-br-md border-b-2 border-r-2 sm:bottom-2 sm:right-2' : 'bottom-2 right-2 h-5 w-5 rounded-br-lg border-b-2 border-r-2 sm:bottom-3 sm:right-3 sm:h-6 sm:w-6'}`} style={{ borderColor: 'color-mix(in srgb, var(--color-cyan) 55%, transparent)' }} />
      ...
      <div className={`relative flex min-h-0 flex-1 flex-col items-center justify-center ${compact ? 'px-1 py-0' : 'px-2 py-1'}`}>
        <span
          className={`font-mono font-800 leading-none tabular-nums ${compact ? 'overview-numeric' : 'fluid-clock'}`}
        ...
        <span className={`text-center font-600 uppercase tracking-wide text-[#b9c7d6] ${compact ? 'mt-0.5 text-[clamp(0.58rem,1.2vh,0.72rem)]' : 'mt-1 text-[clamp(0.7rem,1.6vh,1rem)]'}`}>
          {compact
            ? now.toLocaleDateString('en-GB', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })
            : date}
        </span>
        <span className={`font-mono font-500 tabular-nums text-[#7f93a6] ${compact ? 'mt-0 text-[clamp(0.65rem,1.4vh,0.85rem)]' : 'mt-0.5 text-[clamp(0.85rem,2.2vh,1.35rem)]'}`}>{secs}s</span>
      </div>

      <div className={`relative w-full shrink-0 ${compact ? 'px-1 pb-1' : 'px-1.5 pb-1.5 sm:px-2 sm:pb-2'}`}>
        <div className={`flex flex-wrap items-center justify-center rounded-xl border border-[color-mix(in_srgb,var(--color-cyan)_28%,var(--color-line))] bg-[color-mix(in_srgb,var(--color-cyan)_8%,var(--color-ink-850))] ${compact ? 'gap-1 px-1.5 py-1' : 'gap-1.5 px-2 py-1.5 sm:gap-2 sm:px-3'}`}>
          <StateGlyph state={worst} size={compact ? 12 : 14} />
          <span className={`font-700 tracking-wide ${compact ? 'text-[clamp(0.55rem,1.1vh,0.72rem)]' : 'text-[clamp(0.65rem,1.4vh,0.9rem)]'}`} style={{ color: worst === 'ok' ? 'var(--color-cyan)' : ringColor }}>{statusText}</span>
          <span className={`font-600 text-[#7f93a6] ${compact ? 'text-[clamp(0.52rem,1vh,0.65rem)]' : 'text-[clamp(0.6rem,1.3vh,0.8rem)]'}`}>· {otOccupied ? 'OT Active' : 'OT Idle'}</span>
        </div>
      </div>
    </Panel>
  )
...
function SettingsOverview({ shared, otOccupied }: any) {
  const { now, gases, setGases, proc, setProc, anae, setAnae, setPage } = shared
  const [procControlsOpen, setProcControlsOpen] = useState(false)
  const [anaeControlsOpen, setAnaeControlsOpen] = useState(false)
  const [anaeSetAlertOpen, setAnaeSetAlertOpen] = useState(false)

  const anaeShown = anae.sec > 0 ? anae.sec : anae.target
  const anaeStandby = !anae.run && !anae.sec

  const startAnae = () => {
    setAnae((a: any) => (a.sec > 0 ? { ...a, run: true } : a.target > 0 ? { ...a, sec: a.target, run: true } : a))
  }

  const pauseAnae = () => {
    setAnaeControlsOpen(true)
    setAnae((a: any) => ({ ...a, run: false }))
  }

  const adjustAnaeTarget = (delta: number) =>
    setAnae((a: any) => ({ ...a, target: Math.max(0, Math.min(12 * 3600, a.target + delta)) }))
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col gap-2 overflow-hidden p-2 sm:gap-3 sm:p-3">
      ...
      <div className="relative grid min-h-[9.5rem] flex-[0.58] grid-cols-3 items-stretch gap-2 sm:gap-3">
        <TimeCell
          label="Procedure"
          badge={proc.run ? 'Running' : proc.sec ? 'Paused' : 'Idle'}
          value={fmt(proc.sec)}
          tint="var(--color-gold)"
          onBadgeClick={!proc.run ? () => setProcControlsOpen((o) => !o) : undefined}
          controlsOpen={procControlsOpen}
          controls={
            proc.run ? (
              <MiniBtn icon={<Icon.Pause size={11} />} label="Pause" tint="var(--color-gold)" onClick={() => { setProcControlsOpen(true); setProc((p: any) => ({ ...p, run: false })) }} />
            ) : procControlsOpen ? (
              <>
                <MiniBtn icon={<Icon.Play size={11} />} label="Start" tint="var(--color-gold)" onClick={() => setProc((p: any) => ({ ...p, run: true }))} />
                <MiniBtn icon={<Icon.Reset size={11} />} label="Reset" tint="var(--color-gold)" onClick={() => { setProc({ run: false, sec: 0 }); setProcControlsOpen(false) }} />
              </>
            ) : null
          }
        />
        <TheatreClockPanel now={now} gases={gases} otOccupied={otOccupied} compact />
        <div className="relative h-full min-h-0">
          <TimeCell
            ...
            onBadgeClick={
              !anae.run
                ? () => {
                    if (anaeStandby) setAnaeSetAlertOpen(true)
                    else {
                      setAnaeSetAlertOpen(false)
                      setAnaeControlsOpen((o) => !o)
                    }
                  }
                : undefined
            }
            controlsOpen={anaeControlsOpen || anaeSetAlertOpen}
            controls={
              anae.run ? (
                <MiniBtn icon={<Icon.Pause size={11} />} label="Pause" tint="var(--color-cyan)" onClick={pauseAnae} />
              ) : anaeControlsOpen ? (
                <>
                  <MiniBtn icon={<Icon.Play size={11} />} label="Start" tint="var(--color-cyan)" onClick={startAnae} />
                  <MiniBtn icon={<Icon.Reset size={11} />} label="Reset" tint="var(--color-cyan)" onClick={() => { setAnaeControlsOpen(false); setAnae((a: any) => ({ ...a, run: false, sec: 0 })) }} />
                </>
              ) : null
            }
          />
          <AnaesthesiaSetAlert
            open={anaeSetAlertOpen}
            tint="var(--color-cyan)"
            seconds={anae.target}
            onAdjust={adjustAnaeTarget}
            onDone={() => { setAnaeSetAlertOpen(false); setAnaeControlsOpen(true) }}
            onClose={() => setAnaeSetAlertOpen(false)}
          />
        </div>
      </div>

      {/* ---- main: surgical lights · gases (dominant height) ---- */}
      <div className="relative grid min-h-0 flex-[2.25] grid-cols-1 gap-2 sm:gap-3 lg:grid-cols-[1.65fr_1fr]">
        {/* surgical lights */}
        ...
        {/* gases — 4 tiles per row, no icons; HEPA + filter pressure included */}
        <section className="relative flex min-h-0 flex-col overflow-hidden rounded-2xl border border-[var(--color-line)] bg-[var(--color-ink-900)] p-2 sm:p-3">
          <div className="mb-1.5 shrink-0 text-xs font-800 uppercase tracking-[0.16em] text-[var(--color-fg-soft)] sm:mb-2">Gases &amp; Environment</div>
          <div className="grid min-h-0 flex-1 grid-cols-4 grid-rows-3 gap-1.5 sm:gap-2">
            {gases.map((g: Gas, i: number) => (
              <GasTile
                key={g.key}
                g={g}
                accent={GAS_TILE_ACCENTS[i] ?? 'var(--color-blue)'}
                onClick={() =>
                  setGases((prev: Gas[]) => prev.map((x, xi) => (xi === i ? { ...x, state: cycle[x.state] } : x)))
                }
              />
            ))}
            <ReadoutTile label="IPS" value="Normal" accent="var(--color-ok)" onClick={() => setPage('environment')} />
            <ReadoutTile label="Temp" value="20.4 °C" accent="var(--color-gold)" onClick={() => setPage('environment')} />
            <ReadoutTile label="Humidity" value="48 % RH" accent="var(--color-caution)" onClick={() => setPage('environment')} />
            <ReadoutTile label="Filter Pressure" value="+11 Pa" accent="var(--color-blue)" onClick={() => setPage('environment')} />
            <ReadoutTile label="HEPA" value="Healthy" accent="var(--color-ok)" onClick={() => setPage('environment')} />
          </div>
        </section>
      </div>
    </div>
...
function Settings({ page, setPage, version, setVersion, shared, otOccupied, camActive, modelLabel }: any) {
  ...
  const navItemClass = (active: boolean, locked = false) =>
    `flex w-full shrink-0 items-center rounded-2xl text-left transition-all ${
      navCollapsed ? 'justify-center px-0 py-2' : 'gap-[3px] px-2.5 py-2'
    } ${
      active
  ...
  const navItemStyle = (active: boolean) =>
    active ? { background: 'color-mix(in srgb, var(--color-cyan-dim) 22%, var(--color-ink-900))' } : undefined

  const subNavItemClass = (active: boolean, locked = false) =>
    `flex w-full shrink-0 items-center rounded-xl text-left transition-all ${
      navCollapsed ? 'h-9 justify-center px-0' : 'h-9 gap-[3px] px-2'
    } ${
      active
        ? 'font-700 text-[var(--color-cyan-dim)]'
        : locked
  ...
  const subNavItemStyle = (active: boolean) =>
    active ? { background: 'color-mix(in srgb, var(--color-cyan-dim) 18%, transparent)' } : undefined
  ...
  return (
    <div className="flex h-full min-h-0 flex-col md:flex-row">
      ...
      <nav
        className={`hidden h-full min-h-0 shrink-0 flex-col overflow-hidden border-r border-[var(--color-line)] bg-[var(--color-ink-950)] transition-[width] duration-300 md:flex ${
          navCollapsed ? 'w-14' : 'w-[236px]'
        }`}
      >
        <div className={`flex h-full min-h-0 flex-col gap-1 overflow-hidden py-2.5 ${navCollapsed ? 'px-1.5' : 'px-2.5'}`}>
          {(navCollapsed || page !== 'settings') && (
            <div className={`flex shrink-0 ${navCollapsed ? 'justify-center pb-0.5' : 'justify-end px-0.5'}`}>
              {navCollapsed ? (
                <button
                  type="button"
                  aria-label="Expand menu"
                  onClick={() => setNavCollapsed(false)}
                  className="grid h-7 w-7 place-items-center rounded-lg border border-[var(--color-line)] text-[var(--color-fg-soft)] transition-all hover:bg-[var(--color-ink-750)] hover:text-white active:scale-95"
                >
          ...
          <button
            ...
            className={`${navItemClass(page === 'settings')} text-[13px] font-700`}
            style={navItemStyle(page === 'settings')}
                  ...
                  {groupItems.map((n) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={locked}
                        onClick={() => !locked && setPage(n.id)}
                        className={`${subNavItemClass(active, locked)} text-[12px]`}
          ...
          <div className="shrink-0 border-t border-[var(--color-line)] pt-1">
            <button
              ...
              className={`${subNavItemClass(page === 'admin')} text-[12px]`}
              style={subNavItemStyle(page === 'admin')}
            ...
            >
              ...
              {!navCollapsed && (
                <>
                  <span className="flex-1 truncate leading-snug">Admin</span>
                  <span
                    className="shrink-0 font-mono text-[1rem] font-800 leading-none text-[var(--color-gold)]"
